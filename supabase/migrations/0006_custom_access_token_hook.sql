...
-- tenant_users has RLS enabled (migration 0005). Without a policy for the auth
-- admin role, the hook would read zero rows at token issuance. Allow it to read
-- membership so claims are injected. (Supabase's documented hook pattern.)
create policy tenant_users_authadmin_read on public.tenant_users
