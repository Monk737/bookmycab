-- Tier 2: store the transcript, recording URL, and address-lookup count on each
-- call (for the calls-to-review queue and the transcript/audio viewer).
alter table public.calls
  add column if not exists transcript      text,
  add column if not exists recording_url   text,
  add column if not exists address_lookups integer;
...
create or replace function public.record_voice_call(
  ...
  p_address_lookups integer default null
) returns jsonb
 language plpgsql
 security definer
...
begin
  ...
  insert into public.calls (tenant_id, automation_id, provider_call_id, caller_number, agent_number,
                            ...
                            transcript, recording_url, address_lookups)
  values (p_tenant, p_automation, p_provider_call_id, p_caller, p_agent_number,
          ...
          p_transcript, p_recording_url, p_address_lookups)
...
revoke all on function public.record_voice_call(
  text,uuid,uuid,text,text,timestamptz,timestamptz,integer,text,text,boolean,text,text,numeric,text,text,text,text,text,text,text,text,integer
