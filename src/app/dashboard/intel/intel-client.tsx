  ...
  return (
    <div>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <input value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void search(); }} placeholder="Search transcripts…" className="w-64  border-[3px] border-ink px-2 py-1 text-sm" />
        <button disabled={busy} onClick={() => void search()} className=" border-2 border-ink bg-brut-yellow shadow-brut-sm px-3 py-1.5 text-sm font-medium text-ink disabled:opacity-50">Search</button>
        {!props.isDemo && <button disabled={busy} onClick={() => void analyze()} className=" border-[3px] border-ink px-3 py-1.5 text-sm text-gray-700">Re-score recent</button>}
      ...
      <table className="min-w-full border border-gray-200 text-sm">
        <thead className="bg-gray-50"><tr>{["Customer", "Outcome", "QA", "Flags", "When", ""].map((h) => <th key={h} className="px-3 py-2 text-left font-bold text-gray-700">{h}</th>)}</tr></thead>
        <tbody className="divide-y divide-gray-100">
          ...
          {rows.map((c) => (
            <tr key={c.id}>
              ...
              <td className="px-3 py-2"><span className={c.qa_score == null ? "text-gray-400" : c.qa_score >= 80 ? "text-ink" : c.qa_score >= 60 ? "text-ink" : "text-brut-red-deep"}>{c.qa_score ?? "·"}</span></td>
              ...
              <td className="px-3 py-2 text-right">{!props.isDemo && <button disabled={busy} onClick={() => void flag(c.id, !c.flagged_for_review)} className=" border-[3px] border-ink px-2 py-1 text-xs text-gray-700">{c.flagged_for_review ? "Unflag" : "Flag"}</button>}</td>
            </tr>
          ))}
        </tbody>
      </table>
