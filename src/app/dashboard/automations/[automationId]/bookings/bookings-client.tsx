...
export function BookingsClient({
  ...
  const columns: Column<BookingRow>[] = [
    ...
    },
    {
      ...
      header: "Dispatch Ref",
      render: (row) => (
        <span className="font-mono text-[11px] text-slate-600">{row.dispatchRef ?? "—"}</span>
      ),
    },
    {
      key: "pickupAt",
    ...
    },
    {
      ...
      header: "Customer",
      render: (row) => (
        <span className="font-medium text-slate-900">{row.passengerName ?? "—"}</span>
      ),
    },
    {
      key: "channel",
      ...
      render: (row) =>
        isChannelType(row.channelType) ? (
          ...
          <span className="text-slate-400 text-sm">{row.channelType ?? "—"}</span>
        ),
    },
    {
      key: "pickup",
  ...
  function renderAddressBlock(label: string, addr: unknown) {
    ...
    const rawFields = Object.entries(a)
      ...
      .map(([k, v]) => (
        <div key={k}>
          <dt className="text-slate-500 text-[11px] uppercase tracking-wide">{k}</dt>
          <dd className="text-slate-900 text-sm">{String(v)}</dd>
        </div>
      ));
    return (
      <div className="mb-3">
        ...
        <p className="text-sm text-slate-900 mb-1">{oneLiner}</p>
        {rawFields.length > 0 && (
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <a
          href={csvHref}
          download
          className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 shadow-sm transition-colors duration-150 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-800 cursor-pointer"
        >
          <svg
            aria-hidden="true"
            viewBox="0 0 24 24"
        ...
        {detail && !detailLoading && (
          <div className="space-y-4 text-sm">
            ...
            <section>
              ...
              <dl className="grid grid-cols-2 gap-x-4 gap-y-2">
                <div>
                  ...
                  <dd className="text-slate-900 font-medium">{detail.passengerName ?? "—"}</dd>
                </div>
                <div>
                  <dt className="text-[11px] uppercase tracking-wide text-slate-500">Contact</dt>
                  <dd className="text-slate-900 font-medium">{detail.customerHandle ?? "—"}</dd>
                </div>
                <div>
                  <dt className="text-[11px] uppercase tracking-wide text-slate-500">Passengers</dt>
                  <dd className="text-slate-900 font-medium">{detail.passengerCount ?? "—"}</dd>
                </div>
                <div>
                ...
                <div>
                  <dt className="text-[11px] uppercase tracking-wide text-slate-500">Dispatch Ref</dt>
            ...
            <details className="group">
              <summary className="cursor-pointer text-[11px] font-medium uppercase tracking-wider text-slate-400 hover:text-slate-600 transition-colors duration-150">
                Raw dispatch data (debug)
              </summary>
              <pre className="mt-2 overflow-x-auto rounded bg-slate-50 border border-slate-200 p-3 text-[11px] text-slate-600 max-h-48">
