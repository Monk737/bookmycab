  ...
  return (
    <div>
      <div className="mb-3 flex items-center gap-2">
        {!props.isDemo && (
          <button disabled={busy} onClick={() => call(`${base}/config/versions`, "POST", {})} className=" border-2 border-ink bg-brut-yellow shadow-brut-sm px-3 py-1.5 text-sm font-medium text-ink disabled:opacity-50">
            Snapshot live config as draft
          </button>
        )}
        {msg && <span className="text-sm text-brut-red-deep" role="alert">{msg}</span>}
      </div>
      <table className="min-w-full border border-gray-200 text-sm">
        <thead className="bg-gray-50">
          <tr>{["Version", "Status", "Note", "Published", ""].map((h) => <th key={h} className="px-3 py-2 text-left font-bold text-gray-700">{h}</th>)}</tr>
        </thead>
        <tbody className="divide-y divide-gray-100">
          ...
          {props.versions.map((v) => (
            <tr key={v.id}>
              ...
              <td className="px-3 py-2"><span className={v.status === "published" ? "text-ink" : v.status === "draft" ? "text-ink" : "text-gray-400"}>{v.status}</span></td>
              ...
              <td className="px-3 py-2 text-right">
                {!props.isDemo && (
                  <span className="flex justify-end gap-1">
                    {v.status === "draft" && <button disabled={busy} onClick={() => call(`${base}/config/versions/${v.id}`, "POST")} className=" bg-brut-lime px-2 py-1 text-xs font-medium text-white">Publish</button>}
                    {v.status !== "draft" && <button disabled={busy} onClick={() => call(`${base}/config/rollback`, "POST", { versionId: v.id })} className=" border-[3px] border-ink px-2 py-1 text-xs text-gray-700 hover:bg-gray-50">Roll back to this</button>}
                  </span>
                )}
              </td>
            </tr>
