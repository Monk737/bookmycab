  ...
  return (
    <div>
      <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {props.health.map((h) => (
          <div key={h.adapter} className="rounded-lg border border-gray-200 p-4">
            <div className="flex items-center justify-between">
              <span className="font-semibold capitalize text-gray-800">{h.adapter}</span>
      ...
      <h2 className="mb-2 text-sm font-semibold text-gray-900">Failed dispatches</h2>
      ...
      <table className="min-w-full rounded-lg border border-gray-200 text-sm">
        <thead className="bg-gray-50"><tr>{["When", "Passenger", "Adapter", "Attempt", "Error", ""].map((h) => <th key={h} className="px-3 py-2 text-left font-semibold text-gray-700">{h}</th>)}</tr></thead>
