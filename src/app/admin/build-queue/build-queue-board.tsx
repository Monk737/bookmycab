        ...
        return (
          ...
          >
            <header className="flex items-center justify-between border-b border-zinc-200 px-3 py-2.5">
              <span className="font-mono text-[11px] font-medium uppercase tracking-wider text-zinc-600">
                {stage}
              </span>
              <span className="rounded bg-zinc-200 px-1.5 py-0.5 font-mono text-[11px] font-medium tabular-nums text-zinc-600">
                {stageCards.length}
              </span>
            </header>
...
function Card({ card }: { card: BuildCard }) {
  ...
  return (
    <article
      className={`rounded-md border border-zinc-200 bg-white p-3 shadow-sm transition-opacity ${
        pending ? "opacity-60" : ""
      }`}
    >
      ...
      <p className="mt-0.5 text-sm font-medium text-zinc-900">{card.name}</p>

      <div className="mt-2 flex flex-wrap items-center gap-1.5">
        <span className="rounded border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 font-mono text-[10px] font-medium uppercase tracking-wider text-zinc-500">
          {card.type}
        </span>
        <StatusBadge status={card.status} />
      </div>

      <dl className="mt-2.5 space-y-1 text-xs text-zinc-600">
        <div className="flex items-baseline justify-between gap-2">
          <dt className="text-zinc-400">Engineer</dt>
          <dd className="truncate text-right text-zinc-700">
            {card.engineerEmail ?? "Unassigned"}
          </dd>
        </div>
        <div className="flex items-baseline justify-between gap-2">
          <dt className="text-zinc-400">Go-live</dt>
      ...
      {card.buildNotes && (
        <p className="mt-2 line-clamp-2 text-xs text-zinc-500">{card.buildNotes}</p>
      )}
      ...
      <div className="mt-3 flex flex-col gap-2">
        ...
        {card.buildStage === "UAT" && (
          <button
            ...
            disabled={pending}
            className="cursor-pointer rounded-md bg-emerald-600 px-2 py-1.5 text-xs font-medium text-white outline-none transition-colors hover:bg-emerald-500 focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60"
          >
